...
void path_callback(const nav_msgs::Path &path)
{
  ...
  // (planned_path.Path.size() == 0) || (path.poses.size() != taille_last_path)
 if (nn==0)
 {
    nn=1;
...
std::string robot_name_path;
int main(int argc, char *argv[])
{
  srand(time(0));
  ros::init(argc, argv, "next_goal");
  ros::NodeHandle next_goal;
  next_goal.getParam("/robot_name/odom",robot_name_odom);
  next_goal.getParam("/robot_name/path",robot_name_path);
  ros::Subscriber sub1 = next_goal.subscribe(robot_name_odom, 1000, pose_callback);
  ros::Subscriber sub2 = next_goal.subscribe(robot_name_path, 1000, path_callback);

  ros::Publisher send_points = next_goal.advertise<qingzhou_nav::points>("three_points", 10);
  ...
  while (ros::ok())
  {
    ...
    if (!planned_path.Path.empty())
    {
      ...
      {
        goal_msgs.header.frame_id = robot_name_odom;
        ...
        points.x_p = planned_path.Path[count-2].x;
        points.y_p = planned_path.Path[count-2].y;
        points.x_c = planned_path.Path[count-1].x;
        points.y_c = planned_path.Path[count-1].y;
        points.x_n = planned_path.Path[count].x;
        points.y_n = planned_path.Path[count].y;
        points.x_n_1 = planned_path.Path[count+1].x;
        points.y_n_1 = planned_path.Path[count+1].y;
        // points.x_n_2 = planned_path.Path[count+2].x;
        // points.y_n_2 = planned_path.Path[count+2].y;
        send_points.publish(points);
      }
