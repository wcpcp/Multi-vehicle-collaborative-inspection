...
using namespace cv;
using namespace std;
string robot_name_map;
// ros::param::get("qingzhou_nav/robot_name/map", s);
CleaningPathPlanning::CleaningPathPlanning(costmap_2d::Costmap2DROS *costmap2d_ros)
{
    ...
    //ros::NodeHandle nh;
    private_nh.getParam("/robot_name/map",robot_name_map);
    ...
    int sizex = costmap2d_->getSizeInCellsX(); //获取地图尺寸neuralizedMat.rows/2
...
vector<geometry_msgs::PoseStamped> CleaningPathPlanning::GetPathInROS()
{
    ...
    for (iter = cellvec.begin(); iter != cellvec.end(); iter++)
    {
        ...
        posestamped.header.frame_id = robot_name_map;
...
vector<geometry_msgs::PoseStamped> CleaningPathPlanning::GetBorderTrackingPathInROS()
{
    ...
    // getParam(node);
                            ...
                            theta = atan((curPoint.y - lastPoint.y) / (curPoint.x - lastPoint.x));
...
void CleaningPathPlanning::publishPlan(const std::vector<geometry_msgs::PoseStamped> &path)
{
    ...
    gui_path.header.frame_id = robot_name_map;
...
{
    cellMat = Mat(srcImg.rows / SIZE_OF_CELL, srcImg.cols / SIZE_OF_CELL, srcImg.type()); //cell青州Mat是以之前规定的cell为最小单位重新划分的矩阵
...
{   vector<cellIndex> pathVec;
    ...
    for (i = 0; i < neuralizedMat.rows; i++)
    {
        for (j = 0; j < neuralizedMat.cols; j++)
        {
            if (cellMat.at<uchar>(i, j) == costmap_2d::LETHAL_OBSTACLE)
                ...
                neuralizedMat.at<float>(i, j) = 50.0 ; //这里的1.0/j的用意是什么？这里有没有考虑到j=0时刻的问题呢？   hjr注
        }
    }
   

    if(robot_name_map=="/qingzhou_0/map")
    {
        for (i = neuralizedMat.rows*5/8; i < neuralizedMat.rows; i++)
                    {
                        for (j =0; j < neuralizedMat.cols; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
        ...
        for (i = neuralizedMat.rows*3/8; i < neuralizedMat.rows*5/8; i++)
                    {
                        for (j =neuralizedMat.cols*25/32; j < neuralizedMat.cols; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
                        }
                    }
        for (i = 0; i < neuralizedMat.rows*3/8; i++)
                    {
                        for (j =0; j < neuralizedMat.cols; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
    ...
    if(robot_name_map=="/qingzhou_4/map")
    {
        for (i = neuralizedMat.rows*5/8; i < neuralizedMat.rows; i++)
                    {
                        for (j =0; j < neuralizedMat.cols; j++)
        ...
        for (i = neuralizedMat.rows*3/8; i < neuralizedMat.rows*5/8; i++)
                    {
                        for (j =0; j < neuralizedMat.cols*25/32; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
                        }
                    }
    ...
    if(robot_name_map=="/qingzhou_1/map")
    {
        for (i = neuralizedMat.rows*3/8; i < neuralizedMat.rows; i++)
                    {
                        for (j =0; j < neuralizedMat.cols; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
         ...
         for (i =0 ; i < neuralizedMat.rows*3/8; i++)
                    {
                        for (j = 0; j < neuralizedMat.cols*11/16; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
                        }
                    }
        for (i =0 ; i < neuralizedMat.rows*3/8; i++)
                    {
                        for (j =  neuralizedMat.cols*51/64; j < neuralizedMat.cols; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
                        }
                    }
    }
    if(robot_name_map=="/qingzhou_5/map")
    {
        for (i = neuralizedMat.rows*3/8; i < neuralizedMat.rows; i++)
                    {
                        for (j =0; j < neuralizedMat.cols; j++)
         ...
         for (i =0 ; i < neuralizedMat.rows*3/8; i++)
                    {
                        for (j = 0; j < neuralizedMat.cols*51/64; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
                        }
                    }
    ...
    if(robot_name_map=="/qingzhou_2/map")
    {
         for (i = neuralizedMat.rows*3/8; i < neuralizedMat.rows; i++)
                    {
                        for (j =0; j < neuralizedMat.cols; j++)
         ...
         for (i =neuralizedMat.rows*2/8 ; i < neuralizedMat.rows*3/8; i++)
                    {
                        for (j = 0; j < neuralizedMat.cols*2/4; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
                        }
                    }
        for (i =neuralizedMat.rows*2/8 ; i < neuralizedMat.rows*3/8; i++)
                    {
                        for (j = neuralizedMat.cols*11/16; j < neuralizedMat.cols; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
                        }
                    }
         for (i = 0; i < neuralizedMat.rows*2/8; i++)
                    {
                        for (j =0; j < neuralizedMat.cols; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
     ...
     if(robot_name_map=="/qingzhou_3/map")
    {
         for (i = neuralizedMat.rows*2/8; i < neuralizedMat.rows; i++)
                    {
                        for (j =0; j < neuralizedMat.cols; j++)
                        {
                                neuralizedMat.at<float>(i, j) = -100000.0; 
    ...
    //neuralizedMat.at<float>(pathVec[0].col, pathVec[0].row) = -250;
    return;
}
	...
	for(it=freeSpaceVec_.begin();it!=freeSpaceVec_.end();)
        ...
        */
        ...
        for (int id = 0; id < 8; id++)
        {
            ...
            if(robot_name_map=="/qingzhou_0/map"||robot_name_map=="/qingzhou_4/map"||robot_name_map=="/qingzhou_5/map"||robot_name_map=="/qingzhou_2/map"||robot_name_map=="/qingzhou_3/map")
            {
                    switch (id)
                {
                case 0://上
                    ...
                    v = neuralizedMat_.at<float>(currentPoint.row, currentPoint.col + 1) + c_0 * e+1111;
                ...
                case 2://左
                    ...
                    v = neuralizedMat_.at<float>(currentPoint.row - 1, currentPoint.col) + c_0 * e+3333;
                ...
                case 6://右
                    ...
                    v = neuralizedMat_.at<float>(currentPoint.row + 1, currentPoint.col) + c_0 * e+2222;
            ...
            if(robot_name_map=="/qingzhou_1/map")
            {
                ...
                case 0://上
                    ...
                    v = neuralizedMat_.at<float>(currentPoint.row, currentPoint.col + 1) + c_0 * e+3333;
                ...
                case 4://下
                    ...
                    v = neuralizedMat_.at<float>(currentPoint.row, currentPoint.col - 1) + c_0 * e+2222;
                ...
                case 6://右
                    ...
                    v = neuralizedMat_.at<float>(currentPoint.row + 1, currentPoint.col) + c_0 * e+2000;
        ...
        if (max_v <= 2000) //接下来应该是在处理距离上的关系。
        {
            float dist = 0.0, min_dist = 100000000,theta_=0.0;
            ...
            for (it = freeSpaceVec_.begin(); it != freeSpaceVec_.end(); it++)
            {
                if (neuralizedMat_.at<float>((*it).row, (*it).col) > 0)
                {
                    if (Boundingjudge((*it).row, (*it).col)) //周围是否存在-250的点
                    {
                        ...
                        // GetBorderTrackingPathInCV(resultCV);
                        // vector<bool> visitedVec;
                        //  cv::Point2i lastPoint = resultCV[0];
                        dist = sqrt((currentPoint.row - (*it).row) * (currentPoint.row - (*it).row) + (currentPoint.col - (*it).col) * (currentPoint.col - (*it).col));
                        // theta_ =atan2((lastPoint.x - currentPoint.row),(lastPoint.y- currentPoint.col))-atan2(((*it).row - currentPoint.row),((*it).col - currentPoint.col)) ;
                        // theta_ = theta_ / 180.0 * CV_PI;
                        
                        if ((dist < min_dist))
                        {
                            // if(((theta_<=-11*CV_PI/24)||(theta_>=11*CV_PI/24))){
                                min_dist = dist;
                                min_index = ii;
                            // }
        ...
        neuralizedMat_.at<float>(currentPoint.row, currentPoint.col)=-10000;
        currentPoint = nextPoint;
        pathVec_.push_back(nextPoint);
